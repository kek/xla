...
class AsyncReduceScatterCombiner : public ReduceScatterCombiner {
 public:
  explicit AsyncReduceScatterCombiner(std::string_view strategy = "trivial")
      : ReduceScatterCombiner(/*combine_threshold_in_bytes=*/INT64_MAX,
                              /*combine_threshold_count=*/INT64_MAX,
                              /*combine_by_dim=*/false,
                              /*is_async=*/true, strategy) {}

  absl::string_view name() const override {
    return "async-reduce-scatter-combiner";
  }
};
