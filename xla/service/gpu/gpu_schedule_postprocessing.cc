...
// Returns whether the hlo may invoke gpu-ops directly or indirectly. We only
// check for custom-calls and fusion, because they are the only gpu-ops that
// can be parallel with asynchronous collectives operations (excluding P2P
// operations).
bool MayInvokeGpuOp(HloInstruction* hlo,
                    GpuOpInComputation& GpuOp_in_computation) {
  ...
  return std::any_of(hlo->called_computations().begin(),
                     ...
                     [&](const HloComputation* callee) {
                       return GpuOp_in_computation.find(callee)->second;
                     });
}
...
StatusOr<bool> ProcessComputation(HloSchedule& schedule,
  ...
  for (auto instr_it = all_instructions.begin();
    ...
    if (MayInvokeGpuOp(hlo, GpuOp_in_computation)) {
      async_starts.clear();
      has_GpuOp = true;
      continue;
    }
  ...
  GpuOp_in_computation[computation] = has_GpuOp;
  return changed;
}
