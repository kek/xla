...
#include "third_party/gpus/nccl/include/devcomm.h"
...
StatusOr<NcclComm::Lock> LockMockNcclComm(
  ...
  auto local_comm = AcquireNcclComm(params.run_id, OpId(op_id), local_devices,
                                    local_devices.size(), *unique_id_callback,
                                    local_rank, stream_id, false);
  ...
  return AcquireMockNcclComm(**local_comm, params.run_id, OpId(op_id),
                             std::move(participants), std::move(local_devices),
                             num_local_participants, *unique_id_callback,
                             global_rank, stream_id, false);
}
...
std::shared_ptr<StatusOr<NcclClique::Lock>> AcquireNcclClique(
  ...
  return RendezvousSingle<StatusOr<NcclClique::Lock>>(
      ...
      [&]() -> StatusOr<NcclClique::Lock> {
        ...
        // bool is_local = clique_key.devices().size() ==
        // num_local_participants; TF_RET_CHECK(is_local || (run_id.ToInt() >=
        // clique->run_id));
...
Status InitializeMockNcclCostModel(
  ...
  // Based on which local gpu device participates the input clique, update the
  // peer information.
