...
void ConvertF64ToEf57(absl::Span<const double> input,
  ...
  while (input.size() >= kDoublesPerAvxIteration) {
    __m256d x = _mm256_load_pd(input.data());
    ...
    _mm_storeu_ps(output.data() + 0, _mm_unpacklo_ps(x_hi_f32, x_lo_f32));
    _mm_storeu_ps(output.data() + kFloatsPerSseRegister,
                  _mm_unpackhi_ps(x_hi_f32, x_lo_f32));

    input.remove_prefix(kDoublesPerAvxIteration);
    output.remove_prefix(kFloatsPerAvxIteration);
  }
#endif
#ifdef XLA_HAS_SSE2
